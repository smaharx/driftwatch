...
def explain_cumulative_distribution():
    ...
    print()
    print(
        "KS Test asks: If I drew TWO of these curves (one for training, one for current),"
    )
...
def calculate_ks_test(
    reference_data: np.ndarray, current_data: np.ndarray, verbose: bool = True
    ...
    if verbose:
        ...
        if ks_statistic < 0.05:
            ...
            print("   → Maximum distance between CDFs is tiny")
            print("   → Distributions look very similar")
        elif ks_statistic < 0.15:
            print(f"   KS Statistic = {ks_statistic:.4f} (Small)")
            print("   → Some distance between CDFs")
            print("   → Distributions have some differences")
        elif ks_statistic < 0.3:
            print(f"   KS Statistic = {ks_statistic:.4f} (Moderate)")
            print("   → Meaningful distance between CDFs")
            print("   → Distributions are noticeably different")
        else:
            print(f"   KS Statistic = {ks_statistic:.4f} (Large)")
            print("   → Significant distance between CDFs")
        ...
        if p_value < alpha:
            ...
            explanation = (
                "We fail to reject null hypothesis. Distributions appear similar."
            )
        print(f"   {explanation}")
        print()
...
def visualize_cdf(reference_data, current_data, title):
    ...
    ax2 = axes[1]
    ax2.hist(
        reference_data, bins=30, alpha=0.6, label="Training", color="blue", density=True
    )
    ax2.hist(
        current_data, bins=30, alpha=0.6, label="Current", color="red", density=True
    )
...
ks_stat_2, p_val_2, interp_2 = calculate_ks_test(
    training_small_drift, current_small_drift
)

fig2 = visualize_cdf(
    training_small_drift, current_small_drift, "Scenario 2: Small Drift"
)
...
fig3 = visualize_cdf(
    training_sig_drift, current_sig_drift, "Scenario 3: Significant Drift"
)
plt.savefig(
    "experiments/ks_test_significant_drift_cdf.png", dpi=150, bbox_inches="tight"
)
plt.close()
