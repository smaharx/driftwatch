...
def calculate_jensen_shannon(
    ...
    if verbose:
        ...
        if js_divergence < 0.05:
            ...
            print("   → Distributions have noticeable differences")
        elif js_divergence < 0.5:
            print(f"   JS = {js_divergence:.4f} (Large)")
            print("   → Distributions are quite different")
        else:
        ...
        if js_divergence < 0.1:
            interpretation = "✅ SIMILAR DISTRIBUTIONS (JS < 0.1)"
        elif js_divergence < 0.3:
            interpretation = "⚠️  MODERATE DIFFERENCE (0.1 ≤ JS < 0.3)"
        else:
            interpretation = "🚨 SIGNIFICANT DIFFERENCE (JS ≥ 0.3)"

        print("Decision (typical threshold JS=0.1-0.3):")
...
fig1 = visualize_js_comparison(
    training_no_drift, current_no_drift, "Scenario 1: No Drift"
)
