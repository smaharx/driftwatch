...
def calculate_chi2_test(reference_counts: np.ndarray,
    ...
    if verbose:
        ...
        print(f"Expected Frequencies (if distributions were identical):")
        print("-" * 70)
        print(f"{'Category':<20} {'Training Expected':<20} {'Current Expected':<20}")
        print("-" * 70)
        for i, cat in enumerate(categories):
            print(f"{cat:<20} {expected_freq[0][i]:<20.2f} {expected_freq[1][i]:<20.2f}")
        print("-" * 70)
        print()
        ...
        if chi2_stat < 1:
            ...
            print(f"   → Categories have similar distributions")
        elif chi2_stat < 5:
            print(f"   χ² = {chi2_stat:.4f} (Small)")
            print(f"   → Some difference in distributions")
        elif chi2_stat < 10:
            print(f"   χ² = {chi2_stat:.4f} (Moderate)")
            print(f"   → Noticeable difference in distributions")
        else:
            print(f"   χ² = {chi2_stat:.4f} (Large)")
            print(f"   → Significant difference in distributions")
...
training_categories_1 = np.array([500, 300, 150, 50])  # Electronics, Clothing, Books, Other
current_categories_1 = np.array([500, 300, 150, 50])   # Identical

categories = ['Electronics', 'Clothing', 'Books', 'Other']

chi2_stat_1, p_val_1, interp_1 = calculate_chi2_test(training_categories_1, 
                                                       current_categories_1,
                                                       categories)

fig1 = visualize_categorical_dist(training_categories_1, current_categories_1, 
                                  categories, "Scenario 1: No Drift")
...
chi2_stat_2, p_val_2, interp_2 = calculate_chi2_test(training_categories_2, 
                                                       current_categories_2,
                                                       categories)

fig2 = visualize_categorical_dist(training_categories_2, current_categories_2, 
                                  categories, "Scenario 2: Small Drift")
...
current_categories_3 = np.array([400, 450, 100, 50])    # Major shift in Electronics/Clothing

chi2_stat_3, p_val_3, interp_3 = calculate_chi2_test(training_categories_3, 
                                                       current_categories_3,
                                                       categories)

fig3 = visualize_categorical_dist(training_categories_3, current_categories_3, 
                                  categories, "Scenario 3: Significant Drift")
